...
type CatalogTrustPolicy struct {
	AllowUnsignedCommunity bool
}
...
func VerifyCatalogDigest(m *Manifest) error {
	return VerifyCatalogTrust(m, CatalogTrustPolicy{})
}

func VerifyCatalogTrust(m *Manifest, policy CatalogTrustPolicy) error {
	if m == nil {
		return fmt.Errorf("nil manifest")
	}
	if m.SHA256 == "" {
		if policy.AllowUnsignedCommunity && m.Verification == "community" {
			if m.Signature != "" {
				return fmt.Errorf("community marketplace entry %q cannot include signature without sha256", m.ID)
			}
			return nil
		}
