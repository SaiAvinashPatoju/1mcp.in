// mach1e2e is the 1mcp.in end-to-end test driver. It spawns a mach1
...
func main() {
	...
	if *binPath == "" {
		die(2, "--bin required and mach1 not found in PATH")
	}
...
func runE2E(ctx context.Context, bin, configPath, dbPath string, smokes []Smoke) (*Report, error) {
	...
	if err := cmd.Start(); err != nil {
		return rep, fmt.Errorf("start mach1: %w", err)
	}
	defer func() {
		_ = stdin.Close()
		_ = cmd.Process.Kill()
	...
	for {
		...
		if err != nil {
			if errors.Is(err, io.EOF) {
				return nil, errors.New("mach1 closed stdout unexpectedly")
			}
			return nil, err
		}
...
func findDefaultBin() string {
	for _, p := range []string{"bin/mach1.exe", "bin/mach1"} {
...
func (r *Report) Markdown() string {
	...
	fmt.Fprintf(&b, "# 1mcp.in E2E Report\n\n")
