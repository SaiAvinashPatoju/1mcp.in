// Package clients writes (and removes) the OneMCP entry in the config files
...
// EntryName is the key under which OneMCP registers itself in client configs.
const EntryName = "onemcp"
...
// Connect writes (or replaces) the OneMCP entry in client kind's config file.
...
// Disconnect removes the OneMCP entry from the client config (if present).
