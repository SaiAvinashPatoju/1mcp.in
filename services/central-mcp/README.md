# central-mcp (`centralmcpd`)

The OneMCP central router. Speaks MCP over stdio to one client (e.g. VS Code,
...
> Phase 1 scope only. No sandbox, no semantic routing, no lazy-start yet —
...
```powershell
.\bin\centralmcpd.exe --config services\central-mcp\config.example.json --log debug
```

`stdout` is the MCP wire — do not interleave anything else there. All logs go
to `stderr`.

## Run (registry-backed, what the hub will use)

```powershell
.\bin\centralmcpd.exe --db "$env:APPDATA\OneMcp\registry.db"
```
...
```jsonc
{
  "servers": {
    "onemcp": {
      "command": "C:\\path\\to\\bin\\centralmcpd.exe",
      "args": ["--db", "C:\\Users\\you\\AppData\\Roaming\\OneMcp\\registry.db"]
    }
  }
}
```
...
3. From a second terminal, drive it with any MCP client. Quickest is the
   ...
   ```powershell
   npx @modelcontextprotocol/inspector .\bin\centralmcpd.exe --config services\central-mcp\config.example.json
   ```
...
```
cmd/centralmcpd/      entrypoint + flags
...
- Upstream `tools/call` results are forwarded as raw JSON — we never re-encode
